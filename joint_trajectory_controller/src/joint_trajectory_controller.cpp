    ...
    if (valid_point)
    {
      ...
      for (size_t index = 0; index < dof_; ++index)
      {
        ...
        if (
          ...
          *(rt_is_holding_.readFromRT()) == false &&
          !check_state_tolerance_per_joint(
            state_error_, index, default_tolerances_.goal_state_tolerance[index], false /* show_errors*/))
        {
          ...
          if (default_tolerances_.goal_time_tolerance != 0.0)
          {
            if (time_difference > default_tolerances_.goal_time_tolerance)
            {
              ...
              check_state_tolerance_per_joint(state_error_, index, default_tolerances_.goal_state_tolerance[index], true /* show_errors*/);
            }
          }
        }
      }
